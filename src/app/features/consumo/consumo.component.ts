...
export class ConsumoComponent implements OnInit, OnDestroy {
  ...
  form: any = { cantidad_kg: 0, etapa: 'INICIO' };
  ...
  onLoteChange(): void {
    const l = this.lotes().find(x => x.id === this.form.lote_id);
    if (l) {
      this.form.etapa = l.etapa_actual;
    }
  }

  getConsumoPorPollo(reg: any): number {
    return reg.lote ? (reg.cantidad_kg * 1000) / reg.lote.cantidad_actual : 0;
  }

  openDialog(): void {
    this.form = { cantidad_kg: 0, etapa: 'INICIO' };
    this.dialogVisible.set(true);
  }

  async save(): Promise<void> {
    if (!this.form.lote_id || !this.form.fecha || !this.form.cantidad_kg) {
