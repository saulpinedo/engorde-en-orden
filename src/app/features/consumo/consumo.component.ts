        ...
        <table class="data-table">
          ...
          <tbody>
            @for (reg of registros(); track reg.id) {
              <tr><td>{{ reg.fecha | date:'dd/MM/yyyy' }}</td><td>{{ reg.lote?.nombre || 'Lote' }}</td><td><span class="tag tag-{{ reg.etapa?.toLowerCase() }}">{{ reg.etapa }}</span></td><td><strong>{{ reg.cantidadKg }} kg</strong></td><td>{{ getConsumoPorPollo(reg) | number:'1.2-2' }}g</td></tr>
            }
      ...
      @if (dialogVisible()) {
        <div class="modal-overlay" (click)="dialogVisible.set(false)">
          <div class="modal" (click)="$event.stopPropagation()">
            ...
            <div class="form-group"><label>Lote</label><select [(ngModel)]="form.loteId" (change)="onLoteChange()" class="input-field"><option value="">Seleccionar</option>@for (l of lotes(); track l.id) { <option [value]="l.id">{{ l.nombre || 'Lote ' + l.id?.slice(0,4) }}</option> }</select></div>
            <div class="form-group"><label>Fecha</label><input type="date" [(ngModel)]="form.fecha" class="input-field"/></div>
            <div class="form-group"><label>Cantidad (kg)</label><input type="number" [(ngModel)]="form.cantidadKg" step="0.5" class="input-field"/></div>
...
export class ConsumoComponent implements OnInit, OnDestroy {
  ...
  form: any = { cantidadKg: 0, etapa: 'INICIO' };
  ...
  async loadData(): Promise<void> {
    ...
    for (const l of this.lotes()) {
      const c: any[] = await this.lotService.getConsumos(l.id!);
      c.forEach(x => x.lote = l);
      all.push(...c);
    }
    this.registros.set(all.sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime()));
  }

  onLoteChange(): void {
    const l = this.lotes().find(x => x.id === this.form.loteId);
    if (l) {
      this.form.etapa = l.etapaActual;
    }
  }

  getConsumoPorPollo(reg: any): number {
    return reg.lote ? (reg.cantidadKg * 1000) / reg.lote.cantidadActual : 0;
  }

  openDialog(): void {
    this.form = { cantidadKg: 0, etapa: 'INICIO' };
    this.dialogVisible.set(true);
  }

  async save(): Promise<void> {
    if (!this.form.loteId || !this.form.fecha || !this.form.cantidadKg) {
      alert('Completa los campos');
      return;
    }
    await this.lotService.createConsumo({ ...this.form, fecha: this.form.fecha } as ConsumoDiario);
