...
import { LotService, AntibioticoCatalogo } from '../../core/services/lot.service';
  ...
  template: `
    <div class="page-container">
      <div class="page-header">
        <div><h1>💊 Antibióticos</h1><p class="subtitle">Catálogo de antibióticos disponibles</p></div>
        <button class="btn-primary" (click)="openDialog()">➕ Nuevo Antibiótico</button>
      </div>
      ...
      <div class="card">
        <table class="data-table">
          ...
          <tbody>
            ...
            @empty { <tr><td colspan="4" class="text-center">No hay antibóticos registrados</td></tr> }
          </tbody>
        </table>
      </div>
      ...
      @if (dialogVisible()) {
        <div class="modal-overlay" (click)="dialogVisible.set(false)">
          <div class="modal" (click)="$event.stopPropagation()">
            <h3>{{ editing() ? 'Editar' : 'Nuevo Antibiótico' }}</h3>
...
export class AntibioticosComponent implements OnInit {
  ...
  catalog = signal<AntibioticoCatalogo[]>([]);
  dialogVisible = signal(false);
  editing = signal<AntibioticoCatalogo | null>(null);
  form: any = {};
  ...
  async load(): Promise<void> {
    const data = await this.lotService.getAntibioticosCatalogo();
  ...
  edit(c: AntibioticoCatalogo): void {
    this.editing.set(c);
    this.form = { ...c };
    this.dialogVisible.set(true);
  }
  ...
  async save(): Promise<void> {
    ...
    try {
      if (this.editing()) {
        await this.lotService.updateAntibioticoCatalogo(this.editing()!.id!, this.form);
      } else {
        await this.lotService.createAntibioticoCatalogo(this.form);
      }
  ...
  async delete(c: AntibioticoCatalogo): Promise<void> {
    if (!confirm('¿Eliminar "' + c.nombre + '"?')) return;
    await this.lotService.deleteAntibioticoCatalogo(c.id!);
    await this.load();
  }
}