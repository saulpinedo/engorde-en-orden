...
export class PesajesComponent implements OnInit, OnDestroy {
  ...
  async loadData(): Promise<void> {
    ...
    for (const l of this.lotes()) {
      const p: any[] = await this.lotService.getPesajes(l.id!);
      p.forEach(x => x.lote = l);
      all.push(...p);
    }
  ...
  async save(): Promise<void> {
    ...
    await this.lotService.createPesaje({ ...this.form, fecha: this.form.fecha } as Pesaje);
