...
import { LotService, VitaminaCatalogo } from '../../core/services/lot.service';

@Component({
  selector: 'app-vitaminas',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div class="page-container">
      <div class="page-header">
        <div><h1>🌿 Vitaminas</h1><p class="subtitle">Catálogo de vitaminas disponibles</p></div>
        <button class="btn-primary" (click)="openDialog()">➕ Nueva Vitamina</button>
      </div>
      ...
      <div class="card">
        <table class="data-table">
          ...
          <tbody>
            ...
            @empty { <tr><td colspan="4" class="text-center">No hay vitaminas registradas</td></tr> }
          </tbody>
        </table>
      </div>
      ...
      @if (dialogVisible()) {
        <div class="modal-overlay" (click)="dialogVisible.set(false)">
          <div class="modal" (click)="$event.stopPropagation()">
            <h3>{{ editing() ? 'Editar' : 'Nueva Vitamina' }}</h3>
...
export class VitaminasComponent implements OnInit {
  private lotService = inject(LotService);

  catalog = signal<VitaminaCatalogo[]>([]);
  dialogVisible = signal(false);
  editing = signal<VitaminaCatalogo | null>(null);
  form: any = {};
  ...
  async load(): Promise<void> {
    const data = await this.lotService.getVitaminasCatalogo();
  ...
  async save(): Promise<void> {
    ...
    try {
      if (this.editing()) {
        await this.lotService.updateVitaminaCatalogo(this.editing()!.id!, this.form);
      } else {
        await this.lotService.createVitaminaCatalogo(this.form);
      }
  ...
  async delete(c: VitaminaCatalogo): Promise<void> {
    if (!confirm('¿Eliminar "' + c.nombre + '"?')) return;
    await this.lotService.deleteVitaminaCatalogo(c.id!);
    await this.load();
  }
}